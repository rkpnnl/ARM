% Read ARM Tower Surface Soil Tempearture and Moisture (Eg.,sgpstampE13.b1.20170626.000700.nc)

% Note: Should be interpolated to every 15 minute time intervals for Lidar comparison

function [Met] = ARM_stamp_Proc_CDF(proffile,tol)
...
% try
    soil_moisturee = ncread(proffile,'soil_specific_water_content_east'); %soil_moisturee = soil_moisturee(1,:);
    soil_moisturew = ncread(proffile,'soil_specific_water_content_west'); %soil_moisturew = soil_moisturew(1,:);
    tsoile = ncread(proffile,'soil_temperature_east'); %tsoile = tsoile(1,:);
    tsoilw = ncread(proffile,'soil_temperature_west'); %tsoilw = tsoilw(1,:);
    
% %     Filter the data
    qc_tsoil_e = double(ncread(proffile,'qc_soil_temperature_east'));qc_tsoil_w = double(ncread(proffile,'qc_soil_temperature_west'));
    qc_watcont_e = double(ncread(proffile,'qc_soil_specific_water_content_east'));qc_watcont_w = double(ncread(proffile,'qc_soil_specific_water_content_west'));
